...
func main() {
	...
	for update := range updates {
		...
		if strings.HasPrefix(msg.Text, "/temperature") {
			...
			if len(parts) < 2 {
				reply := tgbotapi.NewMessage(msg.Chat.ID, "Please provide a question. Usage: /ask [question]")
				bot.Send(reply)
				continue
			}
